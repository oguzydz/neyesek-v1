...
class Tarif extends Component {
    ...
    setYemek = () => {
        ...
        if (this.props.hersey === true) {
            ...
            const index = Math.floor(Math.random() * tarif.filter(item => item.recipe.subCategory == category).length);


            
            this.setState({
                yemek: tarif.filter(item => item.recipe.subCategory == category)[index],
                loading: false,
                isFav: this.isFav()
            })
    ...
    refreshRandomsNew = async () => {
        ...
        if (counter === 0) {
            ...
            await AdMobInterstitial.setAdUnitID('ca-app-pub-9786663498474045/3401294709'); // Test ID, Replace with your-admob-unit-id
    ...
    isFav = () => {
        ...
        if (this.props.hersey === true) {
            ...
            const index = Math.floor(Math.random() * tarif.filter(item => item.recipe.subCategory == category).length);
            const isHere = favs.filter(item => item.id == tarif.filter(item => item.recipe.subCategory == category)[index].id);
                    ...
                    <ScrollView style={{ flex: 1 }}>
                        <View style={styles.imageBox}>
                            ...
                            <Image
                                ...
                                source={this.state.yemek.image === '' ? require('../../images/loading_skeleton.png') : { uri: this.state.yemek.image }}
                            />
                        </View>
